      ...
      if (SIM->get_param_enum("type", base)->get() == BX_ATA_DEVICE_DISK) {
        ...
        if ((image_caps & HDIMAGE_HAS_GEOMETRY) != 0) {
          ...
          if (disk_size > BX_HD_THIS channels[channel].drives[device].hdimage->hd_size) {
            BX_ERROR(("error: ata%d-%d: specified_geometry_doesn't_fit_on_disk_image: heads=%d,cyl=%d,spt=%d,disk_size=%d,sect_size=%d      @end", channel, device,heads,cyl,spt,disk_size,sect_size));
