...
{//指令模拟函数MOV_CR0Rd（即修改CR0） 已加日志
  ...
  BX_INFO(("json5日志名=指令模拟函数:MOV_CR0Rd#修改CR0;{CRO:0x%x}",val_32));
...
{//指令模拟函数MOV_CR2Rd（即修改CR2） 已加日志
  ...
  BX_INFO(("json5日志名=指令模拟函数:MOV_CR2Rd#修改CR2;{CR2:0x%x}",(BX_CPU_THIS_PTR cr2) ));
...
{//指令模拟函数MOV_CR3Rd（即修改CR3） 已加日志
  ...
  BX_INFO(("json5日志名=指令模拟函数:MOV_CR3Rd#修改CR3;{CR3:0x%x}",val_32));
...
{//指令模拟函数MOV_CR4Rd（即修改CR4） 已加日志
  ...
  BX_INFO(("json5日志名=指令模拟函数:MOV_CR4Rd#修改CR4;{CR4:0x%x}",val_32));
...
{//指令模拟函数MOV_RdCR0（即读取CR0） 已加日志
  ...
  BX_INFO(( "json5日志名=指令模拟函数:MOV_RdCR0#读取CR0;{CR0:0x%x,'目的寄存器':%d}",val_32,i->dst() ));
...
{//指令模拟函数MOV_RdCR2（即读取CR2） 已加日志
  ...
  BX_INFO(("json5日志名=指令模拟函数:MOV_RdCR2#读取CR2;{CR2:0x%x,'目的寄存器':%d}",(Bit32u) BX_CPU_THIS_PTR cr2,i->dst()));
...
{//指令模拟函数MOV_RdCR3（即读取CR3） 已加日志
  ...
  BX_INFO(("json5日志名=指令模拟函数:MOV_RdCR3#读取CR3;{CR3:0x%x,'目的寄存器':%d}",val_32,i->dst()));
...
{//指令模拟函数MOV_RdCR4（即读取CR4） 已加日志
  ...
  BX_INFO(("json5日志名=指令模拟函数:MOV_RdCR4#读取CR4;{CR4:0x%x,'目的寄存器':%d}",val_32,i->dst()));
