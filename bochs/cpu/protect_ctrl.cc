...
{//LLDT指令( 修改(加载Load) 局部描述符表LDT 寄存器). 指令模拟函数LLDT_Ew（即修改LLDT寄存器） 已加日志 
  ...
  BX_INFO(("LLDT_Ew, %d, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", (BX_CPU_THIS_PTR cpu_mode),_CS_selector_value, _CS_selector_index, EIP, ldtr_selector_json_text.c_str(),ldtr_descriptor_json_text.c_str()));
...
//以下两行，疑似任务切换
  ...
  BX_INFO(("LTR_Ew, %s, %s, %d, 0x%x, 0x%x, 0x%x",tss_selector_json_text.c_str(), tss_descriptor_json_text.c_str(), (BX_CPU_THIS_PTR cpu_mode), _CS_selector_value, _CS_selector_index, EIP));
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::LGDT_Ms(bxInstruction_c *i)//模拟 指令LGDT
{
  ...
  BX_INFO(("LGDT_Ms, %d, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x",BX_CPU_THIS_PTR cpu_mode,_CS_selector_value, _CS_selector_index, EIP, eaddr, base_32, limit_16));
