...
{//LLDT指令( 修改(加载Load) 局部描述符表LDT 寄存器). 指令模拟函数LLDT_Ew（即修改LLDT寄存器） 已加日志 
  ...
  //json5日志名 csv日志=指令模拟函数:LLDT_Ew:修改(加载Load)局部描述符表LDT寄存器,cpu_mode,_CS_selector_value, _CS_selector_index, EIP, ldtr_selector_json_text,ldtr_descriptor_json_text
...
//以下两行，疑似任务切换
  ...
  Bit16u _CS_selector_value=BX_CPU_THIS_PTR sregs[BX_SEG_REG_CS].selector.value;
  Bit16u _CS_selector_index=BX_CPU_THIS_PTR sregs[BX_SEG_REG_CS].selector.index;
  //json5日志名 csv日志=指令模拟函数:LTR_Ew:修改 任务状态寄存器TR,tss_selector_json_text, tss_descriptor_json_text, cpu_mode, _CS_selector_value, _CS_selector_index, EIP
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::LGDT_Ms(bxInstruction_c *i)//模拟 指令LGDT
{
  ...
  Bit16u _CS_selector_value=BX_CPU_THIS_PTR sregs[BX_SEG_REG_CS].selector.value;
  Bit16u _CS_selector_index=BX_CPU_THIS_PTR sregs[BX_SEG_REG_CS].selector.index;
  //json5日志名 csv日志=指令模拟函数:LGDT_Ms:写GDT,cpu_mode,_CS_selector_value, _CS_selector_index, EIP, eaddr, base_32, limit_16
