...
{//str指令(save保存 任务状态寄存器TR). 指令模拟函数STR_Ew（即TR保存） 已加日志 
  ...
  if (i->modC0()) {
    if (i->os32L()) {
      BX_WRITE_32BIT_REGZ(i->dst(), val16);
      //记录日志: //csv日志=指令模拟函数:STR_Ew:保存TR到目的寄存器,TR,目的寄存器 toReg
      BX_INFO(( "指拟:STR_Ew@区32位:存TR; 0x%x; 0x%d" ,val16,i->dst()));
    }
    else {
      BX_WRITE_16BIT_REG(i->dst(), val16);
      //记录日志: //csv日志=指令模拟函数:STR_Ew:保存TR到目的寄存器,TR,目的寄存器 toReg
      BX_INFO(( "指拟:STR_Ew@区16位:存TR; 0x%x; 0x%d" ,val16,i->dst()));
    }
  }
  else {
    bx_address eaddr = BX_CPU_RESOLVE_ADDR(i);
    /* pointer, segment address pair */
    write_virtual_word(i->seg(), eaddr, val16);
      //记录日志: //csv日志=指令模拟函数:STR_Ew:保存TR到目的内存字,TR,目的内存段选择子 toSelector,目的内存段内偏移量 toOffset
    BX_INFO(( "指拟:STR_Ew@区mem:存TR; 0x%x; 0x%d; 0x%x " ,val16, i->seg(), eaddr));
  }
...
{//LLDT指令( 修改(加载Load) 局部描述符表LDT 寄存器). 指令模拟函数LLDT_Ew（即修改LLDT寄存器） 已加日志 
  ...
  std::string ldtr_selector_json_text=BX_CPU_THIS->selector_json_text("ldtr.selector",&(BX_CPU_THIS_PTR ldtr.selector));
  std::string ldtr_descriptor_json_text=BX_CPU_THIS->descriptor_json_text("ldtr.cache",&(BX_CPU_THIS_PTR ldtr.cache));
  ...
  BX_INFO(("~L~=指拟:LLDT_Ew:写LDT; %d; 0x%x; 0x%x; 0x%x; 0x%x; 0x%x"	, (BX_CPU_THIS_PTR cpu_mode),_CS_selector_value, _CS_selector_index, EIP, ldtr_selector_json_text.c_str(),ldtr_descriptor_json_text.c_str()));
...
//以下两行，疑似任务切换
  ...
  std::string task_selector_json_text=BX_CPU_THIS -> selector_json_text("tr.selector",&(BX_CPU_THIS_PTR tr.selector));//TODO 等待确认 tr == task register 吧? 暂且认为是的
  std::string task_descriptor_json_text=BX_CPU_THIS -> descriptor_json_text("tr.cache",&(BX_CPU_THIS_PTR tr.cache));//同上
  ...
  Bit16u _CS_selector_value=BX_CPU_THIS_PTR sregs[BX_SEG_REG_CS].selector.value;
  Bit16u _CS_selector_index=BX_CPU_THIS_PTR sregs[BX_SEG_REG_CS].selector.index;
  //csv日志=指令模拟函数:LTR_Ew:修改 任务状态寄存器TR;task_selector_json_text; task_descriptor_json_text; cpu_mode; _CS_selector_value; _CS_selector_index; EIP
  BX_INFO(("~L~=指拟:LTR_Ew:写TR; %s; %s; %d; 0x%x; 0x%x; 0x%x",task_selector_json_text.c_str(), task_descriptor_json_text.c_str(), (BX_CPU_THIS_PTR cpu_mode), _CS_selector_value, _CS_selector_index, EIP));
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::LGDT_Ms(bxInstruction_c *i)//模拟 指令LGDT
{
  ...
  BX_INFO(("~L~=指拟:LGDT_Ms:写GDT; %d; 0x%x; 0x%x; 0x%x; 0x%x; 0x%x; 0x%x",BX_CPU_THIS_PTR cpu_mode,_CS_selector_value, _CS_selector_index, EIP, eaddr, base_32, limit_16));
