...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::RETnear32_Iw(bxInstruction_c *i)//模拟RET指令
{
  ...
  //json5日志名 csv日志=指令模拟函数:RETnear32_Iw:近32位返回,EIP
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::RETfar32_Iw(bxInstruction_c *i)//模拟RET指令
{
  ...
  if (protected_mode()) {
    ...
    //记录一行日志, EIP、cs_raw
  //json5日志名 csv日志=指令模拟函数:RETfar32_Iw:远32位返回,EIP,cs_raw
...
//   ;
  ...
  //json5日志名 csv日志=函数调用:funcIdEnter:clang插件标记的函数,fId,fId(十六进制),EIP
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::IRET32(bxInstruction_c *i)//模拟IRET指令
{
  ...
  if (protected_mode()) {
    ...
    if (v8086_mode()) {
      ...
      //记录一行日志, EIP、cs_raw
      //json5日志名 csv日志=指令模拟函数:IRET32:32位中断返回,EIP,cs_raw
