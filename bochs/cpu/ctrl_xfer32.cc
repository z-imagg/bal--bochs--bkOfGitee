...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::RETnear32_Iw(bxInstruction_c *i)//模拟RET指令
{
  ...
  BX_INFO( ("指拟:RETnear32_Iw:近32返,0x%x", EIP) );
  BX_NEXT_TRACE(i);
}
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::RETfar32_Iw(bxInstruction_c *i)//模拟RET指令
{
  ...
  if (protected_mode()) {
    ...
    BX_INFO(( "指拟:RETfar32_Iw:远32返,0x%x,0x%x", EIP, cs_raw ));
  }
...
//   ;
  ...
  // json5日志名=函数调用:funcIdEnter#fnIdByClangPlg;@{logName:'funcIdEnter',fId:%d,fIdx:0x%x,EIP:0x%x}
  BX_INFO( ("函调:funcIdEnter:cl插件标函, %d, 0x%x, 0x%x",  funcId,funcId,EIP) );
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::IRET32(bxInstruction_c *i)//模拟IRET指令
{
  ...
  if (protected_mode()) {
    ...
    if (v8086_mode()) {
      ...
      BX_INFO(( "指拟:IRET32:32中返,0x%x,0x%x", EIP, cs_raw ));
    }
  }
