...
BX_CPU_C::iret_protected(bxInstruction_c *i)
{
  ...
  if (cs_selector.rpl == CPL) {//情况2. 同权级中断返回
    ...
    std::string cs_descriptor_json_text=BX_CPU_THIS -> descriptor_json_text("cs_descriptor",&cs_descriptor);
    ...
    std::string ss_descriptor_json_text=BX_CPU_THIS -> descriptor_json_text("ss_descriptor",&ss_descriptor);
    //结束日志行.
    ...
    */
    ...
    BX_INFO(("~L~=返断程共逻:iret_protected__外级返:; %s; %s; 0x%x; %s; %s",cs_selector_json_text.c_str(), cs_descriptor_json_text.c_str(), new_eip, ss_selector_json_text.c_str(),ss_descriptor_json_text.c_str()));
