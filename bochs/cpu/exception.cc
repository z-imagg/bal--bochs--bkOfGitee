...
void BX_CPU_C::protected_mode_int(Bit8u vector, bool soft_int, bool push_error, Bit16u error_code)//80286准保护模式
{
  ...
  case BX_TASK_GATE:{
    ...
    BX_INFO(("protected_mode_int, %d, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", gate_descriptor.type, vector, soft_int, push_error, error_code, tss_selector.index, tss_descriptor.u.taskgate.tss_selector, tss_descriptor.type ));
  ...
  case BX_386_TRAP_GATE:
  {
    ...
    BX_INFO(("protected_mode_int, %d, 0x%lx, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%lx", gate_descriptor.type, EIPorigin, vector, soft_int, push_error, error_code, cs_selector.index, cs_descriptor.u.gate.dest_selector, cs_descriptor.u.gate.dest_offset, cs_descriptor.type, EIP ));
