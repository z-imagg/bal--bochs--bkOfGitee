...
void BX_CPU_C::protected_mode_int(Bit8u vector, bool soft_int, bool push_error, Bit16u error_code)//80286准保护模式
{
  ...
  //csv日志=进入中断指令int处理过程(中断处理过程)中的共用函数:protected_mode_int:80286准保护模式@此行在x区, gate_descriptor.type, vector, soft_int, push_error, error_code, tss_selector.index, tss_descriptor.u.taskgate.tss_selector, tss_descriptor.type 
    BX_INFO(("csv日志=入断程共逻:protected_mode_int:286准保@区%d, %d, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", gate_descriptor.type, gate_descriptor.type, vector, soft_int, push_error, error_code, tss_selector.index, tss_descriptor.u.taskgate.tss_selector, tss_descriptor.type ));
