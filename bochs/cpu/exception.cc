...
void BX_CPU_C::protected_mode_int(Bit8u vector, bool soft_int, bool push_error, Bit16u error_code)//80286准保护模式
{
  ...
  case BX_386_TRAP_GATE:
  {
    ...
    BX_INFO(("json5日志名=中断处理过程提取出的共用函数:protected_mode_int;@{logName:'protected_mode_int','此行在区':%d,'此行在区_描述':'(286INTERRUPT|286_TRAP|386_INTERRUPT|386_TRAP)case__gate_descriptor.type==%d', 'gate_descriptor.type':%d,'此行内容':'load_cs、load_ss、EIP=gate_dest_offset','gate_descriptor.type':%d,EIPorigin:0x%lx,vector:0x%x,soft_int:0x%x,push_error:0x%x,error_code:0x%x,'cs_selector.index':0x%x,'cs_descriptor.u.gate.dest_selector':0x%x,'cs_descriptor.u.gate.dest_offset':0x%x,'cs_descriptor.type':0x%x,EIP:0x%lx}",gate_descriptor.type, gate_descriptor.type, gate_descriptor.type,   EIPorigin, vector,soft_int, push_error, error_code, cs_selector.index,cs_descriptor.u.gate.dest_selector,cs_descriptor.u.gate.dest_offset,cs_descriptor.type,EIP ));
