...
//"[李忠_王晓波]_x86汇编语言从实模式到保护模式-次清晰-170MB-375页全.pdf"(https://www.aliyundrive.com/drive/file/resource/655dcb2d8cc9fbe2f8f04275af1226c93436c652) 纸质页面297页，“在任务切换时，处理执行以下操作：” 的 第11步 “用新的TSS选择子和TSS描述符加载任务寄存器TR” 即 使 任务寄存器TR 指向新的TSS选择子和新的TSS描述符。由此可知，任务切换一定会修改tr, 而以下两行正是修改tr
  ...
  BX_INFO(("json5日志名=业务功能:task_switch#记录日志;@{logName:'task_switch',tss_selector_json_text:%s, tss_descriptor_json_text:%s}",tss_selector_json_text,tss_descriptor_json_text));
  BX_INFO(("业务功能:task_switch#记录日志;@{logName:'task_switch',tss_selector_json_text:%s, tss_descriptor_json_text:%s}",tss_selector_json_text,tss_descriptor_json_text));
