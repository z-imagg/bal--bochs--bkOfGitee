...
{ mkdiskimage 2>/dev/null 1>/dev/null && echo "已经安装mkdiskimage" ; } || { sudo apt install -y syslinux syslinux-common syslinux-efi syslinux-utils ; echo "mkdiskimage安装完毕(mkdiskimage由syslinux-util提供, 但是syslinux syslinux-common syslinux-efi都要安装,否则mkdiskimage产生的此 HD50MB200C16H32S.img 几何参数不对、且 分区没格式化 )"; }

#1. 制作硬盘镜像、注意磁盘几何参数得符合bochs要求、仅1个fat12分区
sudo umount /mnt/hd_img 2>/dev/null ; sudo rm -frv /mnt/hd_img ; rm -fv HD50MB200C16H32S.img
PartitionFirstByteOffset=$(mkdiskimage -o   HD50MB200C16H32S.img 200 16 32)
#  有可能 此命令 并没有正确设置磁盘映像文件10MB.img的几何参数为 200C 16H 32S
# PartitionFirstByteOffset==$((32*512))==16384


#xxd -seek +0X1C3 -len 3 HD50MB200C16H32S.img
#0X1C3:0X0F:15:即16H:即16个磁头, 0X1C4:0X20:32:即32S:即每磁道有32个扇区, 0X1C3:0X27:39:即40C:即40个柱面

#todo 这个0f2027要重新用dg查看
# { test "$(xxd -seek +0X1C3 -len 3  -plain  HD50MB200C16H32S.img)" == "0f2027" && echo "磁盘几何参数指定成功" ; }  || { echo "磁盘几何参数指定失败, 为确认 请用diskgenius专业版打开该x.img查看几何参数" && exit 5; }

echo "注意sfdisk显示磁盘的几何参数与diskgenius的不一致,这里认为sfdisk是错误的，而diskgenius是正确的" && sfdisk --show-geometry HD50MB200C16H32S.img

# parted -s  HD50MB200C16H32S.img mklabel msdos
# parted -s  HD50MB200C16H32S.img mkpart primary fat16 2048s 100%
# parted -s  HD50MB200C16H32S.img set 1 boot on

# mkfs.vfat -F 16 -n C HD50MB200C16H32S.img
...
echo "执行grubinst.exe前md5sum: $(md5sum HD50MB200C16H32S.img)"
...
sshpass -p $win10SshPass scp  -P $win10SshPort HD50MB200C16H32S.img zzz@$win10Host:/HD50MB200C16H32S.img 
...
sshpass -p $win10SshPass ssh -p $win10SshPort zzz@$win10Host "/grubinst_1.0.1_bin_win/grubinst/grubinst.exe /HD50MB200C16H32S.img && echo 'grubinst.exe ok'"

sshpass -p $win10SshPass scp   -P $win10SshPort  zzz@$win10Host:/HD50MB200C16H32S.img  HD50MB200C16H32S.img
#注: $win10Host:/ == D:\msys64, 所以请实现复制 grubinst_1.0.1_bin_win 到 D:\msys64\下

echo "执行grubinst.exe后md5sum: $(md5sum HD50MB200C16H32S.img)"

sudo mkdir /mnt/hd_img
sudo mount -o loop,offset=$PartitionFirstByteOffset HD50MB200C16H32S.img /mnt/hd_img
# sudo losetup --offset $((32*512)) /dev/loop15 HD50MB200C16H32S.img
...
ata0-master: type=disk, path="HD50MB200C16H32S.img", cylinders=40, heads=16, spt=32
boot: c
log: bochsout.txt
mouse: enabled=0
cpu: ips=15000000
